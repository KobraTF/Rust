...
fn main() {
    ...
    //let commands = ["add","check","remove","edit"];
    loop {
...
fn add_person(mut database:HashMap<String,Vec<String>>) -> HashMap<String,Vec<String>> {
    ...
    if index<1 || index == 999999{
        ...
        match database.get_mut(*department) {
            Some(department_data) => {
                department_data.push(name.to_string());
            },
            _ => {
                database.entry(department.to_string()).or_insert(vec![name.to_string()]);
            }
        };
        //let mut department_data = database.entry(department.to_string()).or_insert(vec![name.to_string()]);
    };
    database
}

fn check_database(database:HashMap<String,Vec<String>>) -> HashMap<String,Vec<String>> {
